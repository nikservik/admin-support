        <div class="user-card">
            <div class="mr-3 relative w-12 h-12 text-gray-400">
            ...
            <div class="">
                <a href="/{{ config('admin-support.route') }}/dialog/{{ $dialog->id }}#read">{{ $dialog->name }}</a>
                <span class="text-sm">{{ $dialog->email }}</span>
                @if($dialog->supportMessages->count())
                    <div class="text-gray-500 text-sm mt-1">
                        {{ $dialog->supportMessages[0]->created_at->addHours(3)->format('d.m.Y H:i') }}
                ...
                <div class="text-sm mt-1 hover:no-underline">
                    @if($dialog->message)
                        {!! $dialog->message !!}
                    @elseif($dialog->supportMessages->count())
                        <a href="/{{ config('admin-support.route') }}/dialog/{{ $dialog->id }}#read" class="hover:no-underline">
                            {!! $dialog->supportMessages[0]->message !!}
                        </a>
                    @endif
                </div>
            </div>
